/* Smart21Shop — "Download the app" (installable web app).
 *
 * One small script, used on index.html, shop.html, shop-login.html and
 * shop-app.html. It:
 *   1. links the Smart21Shop manifest + registers its service worker,
 *   2. turns every  [data-shop-install]  element into a working
 ...
 *      (every visit, only on pages with  data-shop-install-popup  on <html>; never inside the installed app).
 *
 * Nothing here touches the rest of the site or the shop app's own code.
 */
(function () {
  'use strict';
  if (window.S21ShopInstall) return;

  var SNOOZE_KEY = 's21-shop-install-snooze';   // "not now" -> ask again in 7 days
  var DONE_KEY = 's21-shop-installed';          // installed on this browser
  var SNOOZE_MS = 7 * 24 * 60 * 60 * 1000;
  var APP_URL = 'shop-app.html';
  ...
  if (!document.querySelector('link[rel="manifest"]')) {
    ...
    l.rel = 'manifest'; l.href = '/shop-manifest.json';
    document.head.appendChild(l);
  }
  if (!document.querySelector('link[rel="apple-touch-icon"][data-shop]')) {
    var a = document.createElement('link');
    a.rel = 'apple-touch-icon'; a.href = '/icons/shop-apple-touch.png'; a.setAttribute('data-shop', '1');
    document.head.appendChild(a);
  }
  if ('serviceWorker' in navigator && location.protocol !== 'file:') {
    window.addEventListener('load', function () {
      navigator.serviceWorker.register('/shop-sw.js', { scope: '/shop-' }).catch(function () {});
    });
  }
  ...
  window.addEventListener('appinstalled', function () {
    ...
    closeSheet();
    refreshButtons();
    toast('Smart21Shop installed — find it on your home screen or app list.');
  });
  ...
  function root() { return document.documentElement; } // survives the shop app rewriting <body>
  ...
  function openSheet(opts) {
    ...
    el.setAttribute('role', 'dialog'); el.setAttribute('aria-modal', 'true'); el.setAttribute('aria-label', 'Download the Smart21Shop app');
    el.innerHTML =
      '<div class="s21i-card">' +
      '<button class="s21i-x" type="button" aria-label="Close">&times;</button>' +
      '<div class="s21i-head"><img src="/icons/shop-192.png" alt="" width="58" height="58"><div><h3>' + esc(opts.title) + '</h3><p>' + esc(opts.sub) + '</p></div></div>' +
      opts.body +
  ...
  function manualSteps() {
    var steps, note = '';
    if (isIOS) {
      steps = '<li>Tap the <b>Share</b> button <span aria-hidden="true">(the square with the arrow)</span> in Safari.</li><li>Scroll and tap <b>Add to Home Screen</b>.</li><li>Tap <b>Add</b>. The Smart21Shop icon appears on your home screen.</li>';
      ...
      steps = '<li>Look for the <b>install icon</b> <span aria-hidden="true">(a small monitor with a down-arrow)</span> at the right end of the address bar.</li><li>Or open the browser <b>&#8942; menu</b> and choose <b>Install Smart21Shop</b>.</li><li>Click <b>Install</b>.</li>';
    }
  ...
  function install(fromPopup) {
    ...
    openSheet({
      title: 'Install Smart21Shop',
      sub: 'Add it to your device in a few taps',
      body: manualSteps(),
      actions: [
        { label: '<i class="fa-solid fa-arrow-up-right-from-square"></i> Open Smart21Shop', href: APP_URL, primary: true, keepOpen: true },
        { label: 'Close' }
      ]
    });
  }
  ...
  // The popup shows on EVERY visit to the shop pages and after every login.
  ...
  function refreshButtons() {
    ...
    document.querySelectorAll('[data-shop-install]').forEach(function (b) {
  ...
  document.addEventListener('click', function (e) {
    var b = e.target.closest && e.target.closest('[data-shop-install]');
    if (!b) return;
    e.preventDefault();
    install(false);
  });

  window.S21ShopInstall = { install: install, showPopup: showPopup, canPrompt: function () { return !!deferred; } };

  /* ---------- auto popup ---------- */
  function boot() {
    refreshButtons();
    if (document.documentElement.hasAttribute('data-shop-install-popup')) {
